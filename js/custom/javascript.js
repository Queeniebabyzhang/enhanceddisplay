window.onload=function(){
	...
	for(var i=0;i<32;i++){
		...
		var nav = document.getElementById("navbartoggle");
		nav.onblur = function(){
			 $("#head-collapsable-nav").collapse('hide');
		}
	...
	for(var i=0;i<checkarr.length;i++){
		...
		checkarr[i].onclick=function(){
           if(this.checked){
			   this.style.backgroundImage='url("images/mobile/empty.png")';
			   this.checked=false;
			   imagebox[this.index].style.display="none";
		   }else{
			   this.style.backgroundImage='url("images/mobile/check.png")';
			   this.checked=true;
